...
// Threshold for using the chunked summary flow (in bytes). 1MB.
...
export function FileSummarizer() {
  ...
  const handleSummarize = async () => {
    ...
    try {
        ...
        let result;
        if (useChunking) {
          toast({ title: 'Large file detected', description: 'Using deep summarization. This may take a few minutes.'});
          result = await generateChunkedSummary({
            parts,
            fileType,
          });
        } else {
          result = await summarizeUploadedFile({
            parts,
            fileType,
            summaryType
          });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <RadioGroup value={summaryType} onValueChange={(value: SummaryType) => setSummaryType(value)} className="flex flex-col sm:flex-row gap-4">
